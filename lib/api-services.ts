...
export const chatsApi = {
  ...
  createRoom: (otherUserId: string) =>
    request<{ id: string; created_at: string; name?: string }>("/chats/rooms", {
      method: "POST",
      body: JSON.stringify({ other_user_id: otherUserId }),
    }),
  getMessages: (roomId: string) =>
    request<any[]>(`/chats/rooms/${roomId}/messages`),
  sendMessage: (
    roomId: string,
    text: string,
    kind: "text" | "announce" = "text",
  ) =>
    request(`/chats/rooms/${roomId}/messages`, {
      method: "POST",
...
export const dogsApi = {
  ...
  addDog: (data: {
    ...
    age?: number;
    weight?: number;
  }) => request("/dogs/mine", { method: "POST", body: JSON.stringify(data) }),
  updateDog: (
    dogId: string,
    data: { name?: string; breed?: string; age?: number; weight?: number },
  ) =>
    request(`/dogs/mine/${dogId}`, {
      method: "PUT",
      body: JSON.stringify(data),
    }),
  deleteDog: (dogId: string) =>
    request(`/dogs/mine/${dogId}`, { method: "DELETE" }),
};
...
export const usersApi = {
  ...
  discover: (search?: string) =>
    request<any[]>(
      `/users/discover${search ? `?search=${encodeURIComponent(search)}` : ""}`,
    ),
};
